...
import {IfElseResolver} from "../src/classLibrary/ParsingStrategy/IfElseResolver";
...
describe("IfElseResolver", () => {
	...
	let ifElseParser: IfElseResolver = new IfElseResolver(util);
	describe("Count", () => {
		it("should count the number of occurrences of each key in every file and return a map", () => {
			...
			let actual =
				ifElseParser.Count(flagObject, files, new Map([]), true)
					.SortByKey(SortType.AtoZ);
			...
			let oldActual = ifElseParser.Count(flagObject, files, oldMap, true)
				.SortByKey(SortType.AtoZ);
			let oldExpected = new Map([
				["a.b.c", 2],
				["a.b.d", 13],
				["a.e", 2],
				["a.f", 4],
			...
			oldActual.Arr().should.deep.equal(oldExpected.Arr());
