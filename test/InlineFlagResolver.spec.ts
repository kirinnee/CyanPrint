import {InlineFlagResolver} from "../src/classLibrary/ParsingStrategy/InlineFlagResolver";
...
let inlineFlagParser: InlineFlagResolver = new InlineFlagResolver(util, ["//"]);
...
describe("InlineFlagResolver", () => {
	...
	describe("ResolveContents", () => {
		it("should remove false flags and remove injected code", () => {
			...
			inlineFlagParser.ResolveFiles(simpleFlagObject, files)
...
eget finibus venenatis.`;
			...
			let map: Map<string, boolean> = util.FlattenFlagObject(simpleFlagObject).MapKey(inlineFlagParser.ModifyFlagKeys);
