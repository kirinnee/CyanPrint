...
export class InverseInlineFlagResolver implements ParsingStrategy {
	...
	CountPossibleUnaccountedFlags(files: FileSystemInstance[]): string[] {
		...
		return files
			...
			.Map(f =>
				(f as IFile).content
					.LineBreak()
					.Map(s => s.Match(flag).Map(s => `${s}:${f.relativePath}`))
					.Flatten()
			)
			.Flatten()
			.concat(
				files.Map((f: FileSystemInstance) => f.destinationAbsolutePath.Match(flag)).Flatten()
			) as string[];
	}
	...
	ResolveFiles(wFlag: object, files: FileSystemInstance[]): FileSystemInstance[] {
		...
		let map: Map<string, boolean> = this.util
			.FlattenFlagObject(wFlag)
			.MapKey(this.ModifyFlagKeys)
	...
	private RemoveArray(map: Map<string, boolean>): string[] {
		let nestedArr: string[][] = this.comments.Map((c: string) => map.Keys().Map((k: string) => c + k));
		return nestedArr.Flatten();
	}
}